...
import { BadgeCheck, CalendarDays, CheckCircle2, Fingerprint, Home } from 'lucide-react';
    ...
    return (
      <div className="relative min-h-[calc(100vh-80px)] overflow-hidden px-4 pt-2 pb-10 sm:px-6">
        ...
        <div className="relative mx-auto max-w-6xl space-y-8">
          <GlassPanel className="px-8 py-4 sm:px-10 sm:py-6">
            <div className="mb-2 flex w-full flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
              <div className="inline-flex w-fit items-center gap-1.5 rounded-full border border-[var(--tenant-secondary)]/35 bg-white/55 px-4 py-2 text-xs font-black uppercase tracking-widest text-[var(--tenant-primary)] shadow-sm backdrop-blur-xl">
                <Home className="h-4 w-4" />
                {roleLabel} Dashboard
              </div>
              {currentTime && (
                <div className="inline-flex w-fit items-center gap-2 rounded-full border border-white/65 bg-white/55 px-4 py-2 text-xs font-bold text-slate-800 shadow-sm backdrop-blur-xl">
            ...
            <div className="mt-8 space-y-5 border-t border-white/20 pt-8">
              <div className="px-1">
                <p className="text-[10px] font-black uppercase tracking-[0.25em] text-[var(--tenant-primary)]">Public Election Dashboard</p>
                <h2 className="mt-1 text-2xl font-black tracking-tight text-slate-950 md:text-3xl">Live Election Overview</h2>
              </div>
                ...
                {isVotingPhaseActive ? (
                  <div className="mt-5 grid gap-4 sm:grid-cols-3">
                    {displayedCandidates.map((cand, index) => (
                      <div key={index} className="rounded-2xl border border-slate-100 bg-white/80 p-4 shadow-sm transition duration-300 hover:border-[var(--tenant-primary)]/20 hover:shadow-md">
                        <div className="flex items-center justify-between text-xs font-black">
                          <span className="max-w-[130px] truncate text-slate-700" title={cand.name}>{cand.name}</span>
          ...
          <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
            ...
            <StatCard icon={Fingerprint} label="Account" value={roleLabel} />
