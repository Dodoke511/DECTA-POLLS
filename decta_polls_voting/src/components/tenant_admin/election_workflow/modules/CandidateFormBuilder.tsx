...
import {
  ...
  List, Circle, CheckSquare, Minus, Eye
} from 'lucide-react';
...
function FieldCardBuild({ field, index, dragIndex, onUpdate, onDelete, onDragStart, onDragOver, onDrop }: {
...
function FieldPreview({ field }: { field: FormFieldState }) {
  const vr = field.validation_rules;
...
export const DynamicFormBuilder = forwardRef(({ electionId, toolName = 'candidate_application', title = 'Candidate Application Form' }: { electionId: string, toolName?: string, title?: string }, ref) => {
  ...
  useImperativeHandle(ref, () => ({
    save: async () => {
      try {
        // Map snake_case internal state to camelCase API payload
        ...
        if (!res.ok) {
          ...
          console.error("Form save API error:", errorData);
  ...
  useEffect(() => {
    fetch(`/api/get_form?electionId=${electionId}&toolName=${toolName}`)
      ...
      .then(({ fields: fetched }) => {
        if (fetched?.length > 0) {
          // Map camelCase database fields back to snake_case UI state
          setFields(fetched.map((f: any) => ({
            id: f.id,
  ...
  return (
    <div className="mt-5 pt-5 border-t border-white/5">
      ...
      {!isLoading && activeTab === 'build' && (
        <div className="space-y-2">
          {fields.length === 0 && <div className="flex items-center justify-center h-20 rounded-xl border border-dashed border-white/10 bg-white/2 text-white/28 text-[12px]">No fields yet — click + Add Field to start.</div>}
      ...
      {!isLoading && activeTab === 'preview' && (
        <div className="rounded-xl border border-white/8 bg-[#0D0A1A]/40 p-6">
          {fields.length === 0 ? <p className="text-center text-[12px] text-white/28">Add fields in Build mode.</p> : (
            <div className="max-w-lg space-y-5">
              <div><h3 className="text-[15px] font-semibold text-white/85">Candidate Application</h3><p className="text-[11px] text-white/35 mt-0.5">Preview row</p></div>
              {fields.map(f => <FieldPreview key={f._key} field={f} />)}
            </div>
          )}
        </div>
      )}
