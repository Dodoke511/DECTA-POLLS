            ...
            <div className="flex flex-col gap-3 w-full md:flex-row md:items-center md:w-auto">
              <div className="relative w-full md:w-64">
                ...
                <input
                  ...
                  className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-2 text-sm focus:outline-none focus:border-[var(--tenant-primary)] transition-all"
                  value={searchQuery}
              ...
              <div className="flex bg-white/5 p-1 rounded-xl border border-white/10 w-full md:w-auto">
                {(['ALL', 'PENDING_VERIFICATION', 'APPROVED'] as const).map((f) => (
                  <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={`flex-1 md:flex-none px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${filter === f ? 'bg-[var(--tenant-primary)] text-white shadow-lg' : 'text-white/40 hover:text-white/60'
                      }`}
                  >
